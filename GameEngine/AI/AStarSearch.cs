...
namespace CPI311.GameEngine
{
    public class AStarSearch
    {
        ...
        public AStarSearch(int rows, int cols)
        {
            ...
            {
                for (int c = 0; c < Cols; c++) Nodes[r, c] = new AStarNode(c, r, new Vector3(c, 0, r));
            }
        }
        ...
        public void Search()
        {
            ...
            while (openList.Count > 0) // openlist is not empty
            {
                ...
                // Add four neighbors
                if (node.Row < Rows - 1) AddToOpenList(Nodes[node.Row+1, node.Col], node);
                if (node.Row > 0) AddToOpenList(Nodes[node.Row-1, node.Col], node);
                if (node.Col < Cols - 1) AddToOpenList(Nodes[node.Row, node.Col+1], node);
                if (node.Col > 0) AddToOpenList(Nodes[node.Row, node.Col-1], node);
            }
        }
        ...
        private AStarNode GetBestNode()
        {
            ...
            if (openList.ElementAt(0).Value.Count == 0) openList.Remove(node.Cost + node.Heuristic);
            node.Closed = true;
            return node;
        }
